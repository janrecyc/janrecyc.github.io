...
- **ออกแบบไอคอนแอพใหม่** (`resources/android-icon/`) — เดิมใช้ไอคอน default ของ Capacitor เพราะไม่มีที่ไหนสั่งเปลี่ยน ตอนนี้มีไอคอนของแบรนด์เองแล้ว (วงลูกศรหมุนสีส้มบนพื้นกรมท่า) ครบทั้ง legacy icon และ adaptive icon (Android 8.0+) ทุก density — เห็นตัวอย่างได้ที่ `resources/icon-preview.png` — workflow จะคัดลอกเข้าไปแทนที่ไอคอน default ให้อัตโนมัติทุกครั้งที่ build (ขั้นตอน "Apply custom app icon")
