...
// Wrapped in an IIFE — buy.js and sell.js were separate ScrapPOS pages
// that never coexisted, and both declared the SAME top-level variable/
// function names (cart, ITEMS, openKgModal, kpNum, ...). Now that both
// load together on shop.html, a bare `let cart` in each file would be a
// SyntaxError (redeclaring the same global twice), and bare `function`
// names would silently overwrite each other. Only initBuyTab/applyBuySearch
// (called by shop-page.js) and the handful of functions referenced by
// onclick="..." in shop.html's static overlay markup are exposed, via the
// returned BuyTab object below — everything else stays private to this file.
const BuyTab = (function () {
...
function openKgModal(id) {
  ...
  document.getElementById('bkg-item-icon').innerHTML  = renderIcon(item.icon, item.cat, 26);
  document.getElementById('bkg-item-name').textContent  = item.name;
  document.getElementById('bkg-item-price').textContent = `฿${Number(item.buy_price).toFixed(2)} / ${item.unit||'กก.'}`;
  ...
  document.getElementById('bkg-backdrop').classList.add('open');
  document.getElementById('bkg-modal').classList.add('open');
  document.body.style.overflow = 'hidden';
}

function closeKgModal() {
  document.getElementById('bkg-backdrop').classList.remove('open');
  document.getElementById('bkg-modal').classList.remove('open');
...
function setMode(mode) {
  currentMode = mode;
  document.getElementById('b-mode-btn-kg').classList.toggle('active', mode === 'kg');
  document.getElementById('b-mode-btn-piece').classList.toggle('active', mode === 'piece');
  if (mode === 'kg') {
    document.getElementById('bkg-display-label').textContent = 'น้ำหนัก';
    document.getElementById('kg-unit-label').textContent    = 'กก.';
  } else {
    document.getElementById('bkg-display-label').textContent = 'จำนวน';
...
function updateKgDisplay() {
  ...
  const lblEl2 = document.getElementById('bkg-display-label');
  ...
  const tapeEl = document.getElementById('bkg-tape');
  tapeEl.innerHTML = kgEntries.map((e, i) => `
    <div class="kg-tape-entry">
      ...
      <button class="kg-tape-del" onclick="BuyTab.tapeDelete(${i})"><i class="ph ph-x"></i></button>
    </div>
  `).join('');
  tapeEl.scrollTop = tapeEl.scrollHeight;

  document.getElementById('bkg-display-num').textContent = kgBuffer || '0';
  ...
  document.getElementById('bkg-subtotal').textContent = sub > 0 ? `฿${sub.toFixed(2)}` : '฿0';

  const btnEl = document.getElementById('bkg-confirm-btn');
  const lblEl = document.getElementById('bkg-confirm-label');
  const valid = total > 0;
...
function renderCartPanel() {
  ...
  const countEl = document.getElementById('buy-cart-count');
  ...
  listEl.innerHTML = entries.map(c => `
    <div class="cart-item-chip" onclick="BuyTab.openKgModal(${c.item.id})">
      ...
      <button class="cart-item-chip-remove" onclick="event.stopPropagation();BuyTab.removeFromCart(${c.item.id})">
        <i class="ph ph-x"></i>
      </button>
    </div>
  `).join('');
...
function renderGrid() {
  ...
  grid.innerHTML = list.map(item => {
    ...
    return `
    <div class="item-card ${inCart?'selected':''}" onclick="BuyTab.openKgModal(${item.id})">
      ...
      <div class="card-bottom">
        ...
        <button class="card-add-btn" onclick="event.stopPropagation();BuyTab.openKgModal(${item.id})">
...
// ── Events (bkg-modal swipe-to-close — safe to wire at file-load time
//    since #bkg-modal is static markup in shop.html, not part of the
//    dynamically injected mount below) ──
let touchStartY = 0;
let swipeFromHandle = false;
const kgHandleEl = document.querySelector('#bkg-modal .kg-handle');
const kgHeaderEl = document.querySelector('#bkg-modal .kg-header');
...
document.getElementById('bkg-modal').addEventListener('touchstart', e => {
  if (!e.target.closest('.kg-handle') && !e.target.closest('.kg-header')) swipeFromHandle = false;
}, {passive:true});
document.getElementById('bkg-modal').addEventListener('touchmove', e => {
    ...
    <div class="cart-panel" id="cart-panel">
      <div class="cart-header">
        <div class="cart-title">
          ...
          ตะกร้ารับซื้อ
          <span class="cart-count-badge" id="buy-cart-count">0</span>
        </div>
...
// bar — search-toggle-mount/search-mount, same one every shop.html tab
// uses) whenever this tab is active, instead of this tab having its own
// separate search box.
function applyBuySearch(text) {
  query = (text || '').trim();
  if (document.getElementById('item-grid')) renderGrid();
}

  return {
    initBuyTab, applyBuySearch,
    openKgModal, closeKgModal, tapeDelete, setMode,
    kpNum, kpDot, kpBack, kpMultiply, kpPlus, kpClear, confirmKg,
    removeFromCart, clearCart, confirmCart,
    skipReceipt, openReceipt, closeReceipt, sendLine, printReceipt
  };
})();
