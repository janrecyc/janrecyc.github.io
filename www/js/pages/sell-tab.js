// ══════════════════════════════════════════
//  pages/sell-tab.js — the "ขายออก" tab's content, rendered into
//  shop.html's #shop-mount by shop-page.js's tab router. Ported
//  from ScrapPOS's own sell.html/sell.js, same overall treatment
//  as pages/buy-tab.js (see its header comment for the general
//  pattern — mount-injected catalogue, static overlays for
//  anything modal, no more page-level header/logout).
//
//  Wrapped in its own IIFE (like buy-tab.js) for the same reason:
//  buy.js and sell.js were separate ScrapPOS pages that both
//  declared identical top-level names (cart, ITEMS, openKgModal,
//  ...). Only initSellTab/applySellSearch (called by shop-page.js)
//  and the functions referenced by onclick="..." in shop.html's
//  static overlay markup are exposed, via the returned SellTab
//  object below.
//
//  The kg-entry modal here is functionally almost identical to
//  buy-tab.js's, but with ITS OWN element ids (skg-*, s-mode-btn-*)
//  — buy's and sell's kg-modals are BOTH static overlays that sit
//  outside #shop-mount (so, unlike the mount-swapped catalogue
//  markup, both exist in the DOM at once), and originally shared
//  the exact same ids (#skg-modal, #skg-tape, ...) since ScrapPOS
//  never had them on the same page. Reusing ids here would be
//  a duplicate-id bug, so sell's copy in shop.html is fully
//  separate markup, not the same physical modal buy-tab.js uses.
// ══════════════════════════════════════════

const SellTab = (function () {
...
function setMode(mode) {
  ...
  document.getElementById('s-mode-btn-kg').classList.toggle('active', isKg);
  document.getElementById('s-mode-btn-piece').classList.toggle('active', !isKg);
  document.getElementById('skg-display-label').textContent = isKg ? 'น้ำหนัก' : 'จำนวน';
...
function openKgModal(id) {
  ...
  document.getElementById('skg-item-icon').innerHTML   = renderIcon(item.icon, item.cat, 26);
  document.getElementById('skg-item-name').textContent  = item.name;
  document.getElementById('skg-item-price').textContent = `฿${Number(item.sell_price).toFixed(2)} / ${item.unit||'กก.'}`;
  ...
  updateKgDisplay();
  document.getElementById('skg-backdrop').classList.add('open');
  document.getElementById('skg-modal').classList.add('open');
  document.body.style.overflow = 'hidden';
}

function closeKgModal() {
  document.getElementById('skg-backdrop').classList.remove('open');
  document.getElementById('skg-modal').classList.remove('open');
...
function updateKgDisplay() {
  ...
  const lblEl2 = document.getElementById('skg-display-label');
  ...
  // ── tape ──
  const tapeEl = document.getElementById('skg-tape');
  tapeEl.innerHTML = kgEntries.map((e, i) => `
    <div class="kg-tape-entry">
      ...
      <button class="kg-tape-del" onclick="SellTab.tapeDelete(${i})"><i class="ph ph-x"></i></button>
    </div>
  `).join('');
  tapeEl.scrollTop = tapeEl.scrollHeight;

  // ── buffer display ──
  document.getElementById('skg-display-num').textContent = kgBuffer || '0';

  // ── subtotal ──
  const sub = Math.round(total * price * 100) / 100;
  document.getElementById('skg-subtotal').textContent = sub > 0 ? `฿${sub.toFixed(2)}` : '฿0';
  ...
  // ── confirm button ──
  const btnEl = document.getElementById('skg-confirm-btn');
  const lblEl = document.getElementById('skg-confirm-label');
...
function renderCartList() {
  ...
  listEl.innerHTML = keys.map(id => {
    ...
    <div class="cart-row">
      ...
      <button class="cart-row-weight-btn" onclick="SellTab.editFromCart(${id})">
      ...
      <button class="cart-row-del" onclick="SellTab.removeFromCart(${id})">
...
function renderGrid() {
  ...
  grid.innerHTML = list.map(item => {
    ...
    return `
    <div class="item-card ${inCart?'selected':''} ${oos?'out-of-stock':''}" onclick="SellTab.openKgModal(${nid})">
      ...
      <div class="card-bottom">
        ...
        <button class="card-add-btn" onclick="event.stopPropagation();SellTab.openKgModal(${nid})">
...
function renderFactoryEditList(factories) {
  ...
  list.innerHTML = factories.map((f, i) => `
    <div class="fedit-item" id="fedit-row-${i}" data-id="${f.id}">
      ...
      <button class="fedit-item-del" onclick="SellTab.deleteFactoryRow(${i})" title="ลบ">
        <i class="ph ph-trash"></i>
      </button>
    </div>
  `).join('') + `
    <button class="fedit-add-btn" onclick="SellTab.addFactoryRow()">
...
// ── Events (skg-modal/cart-modal swipe-to-close — safe to wire at file-
//    load time since both are static markup in shop.html, not part of
//    the dynamically injected mount below) ──
function swipeClose(modalId, closeFn) {
  let ty = 0;
  const el = document.getElementById(modalId);
  if (!el) return;
  el.addEventListener('touchstart', e => { ty = e.touches[0].clientY; }, {passive:true});
  el.addEventListener('touchmove',  e => { if (e.touches[0].clientY - ty > 70) closeFn(); }, {passive:true});
}
swipeClose('skg-modal',  closeKgModal);
...
// ══════════════════════════════════════════
//  เข้าทาง shop-page.js เมื่อเลือกแท็บ "ขายออก"
// ══════════════════════════════════════════
function initSellTab(mountId) {
  const mount = document.getElementById(mountId);
  if (!mount) return;

  mount.innerHTML = `
    <div class="sell-tab-topbar">
      <button class="icon-btn" id="sell-refresh-btn" title="รีเฟรช">
        <i class="ph ph-arrows-clockwise"></i>
      </button>
      <button class="icon-btn cart-fab" id="sell-cart-fab" title="รายการส่ง">
  ...
  document.getElementById('sell-refresh-btn').addEventListener('click', loadItems);
  document.getElementById('sell-cart-fab').addEventListener('click', openCartModal);

  loadItems();
  loadShopProfileForReceipt();
}

// Called by shop-page.js's onInput handler (the shared template search
// bar) whenever this tab is active, instead of this tab having its own
// separate search box.
function applySellSearch(text) {
  query = (text || '').trim();
  if (document.getElementById('item-grid')) renderGrid();
}
  return {
    initSellTab, applySellSearch,
    openKgModal, closeKgModal, tapeDelete, setMode,
    kpNum, kpDot, kpBack, kpMultiply, kpPlus, kpClear, confirmKg,
    openCartModal, closeCartModal, clearCart, removeFromCart, editFromCart,
    saveSellTransaction,
    skipSellReceipt, openSellReceipt, closeSellReceipt, printReceipt, sendLine,
    openFactoryEdit, closeFactoryEdit, deleteFactoryRow, addFactoryRow, saveFactoryEdit
  };
})();
