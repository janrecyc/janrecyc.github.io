...
//
//  ต้องโหลดหลัง js/scrap-db/db.js (ใช้ ScrapDB.dbQuery/ScrapDB.dbRun —
//  ฐานข้อมูล 'scrapposdb' คนละตัวกับ 'shopapp' ที่ระบบ login ของ
//  เทมเพลตหลักใช้ ดู db.js หัวไฟล์สำหรับเหตุผลที่แยกกัน) การเรียกใช้
//  ในไฟล์นี้ (`localRest()` / `sbFetch()` ในแต่ละหน้า) ยังคงชื่อ/รูป
//  แบบเดิมทุกอย่าง เปลี่ยนแค่ตัวที่มันเรียกไปหา SQLite ข้างใต้เท่านั้น
// ══════════════════════════════════════════
...
async function _selectRows(table, params) {
  ...
  return ScrapDB.dbQuery(sql, values);
}
...
async function _insertRow(table, body) {
  ...
  const res = await ScrapDB.dbRun(sql, values);
  const newId = res && res.changes && res.changes.lastId;
  if (newId != null) {
    const rows = await ScrapDB.dbQuery(`SELECT * FROM ${table} WHERE id = ?`, [newId]);
    return rows;
  }
  return [];
}
...
async function _updateRows(table, params, body) {
  ...
  await ScrapDB.dbRun(sql, [...setValues, ...whereValues]);
  return _selectRows(table, params);
}

async function _deleteRows(table, params) {
  const { sql: whereSql, values } = _buildWhere(params);
  await ScrapDB.dbRun(`DELETE FROM ${table} ${whereSql}`, values);
  return null;
}
...
async function _rpcRecordBuy(p) {
  ...
  for (const l of lines) {
    await ScrapDB.dbRun(`UPDATE items SET stock_qty = COALESCE(stock_qty,0) + ? WHERE id = ?`, [l.qty, l.item_id]);
  }
  return { success: true };
}
...
async function _rpcRecordSell(p) {
  ...
  for (const l of lines) {
    await ScrapDB.dbRun(`UPDATE items SET stock_qty = MAX(0, COALESCE(stock_qty,0) - ?) WHERE id = ?`, [l.qty, l.item_id]);
  }
  return { success: true };
}
...
async function _rpcRecordSort(p) {
  ...
  await ScrapDB.dbRun(`UPDATE items SET stock_qty = MAX(0, COALESCE(stock_qty,0) - ?) WHERE id = ?`, [fromKg, p.p_from_item_id]);
  for (const r of results) {
    await ScrapDB.dbRun(`UPDATE items SET stock_qty = COALESCE(stock_qty,0) + ? WHERE id = ?`, [r.kg, r.item_id]);
  }
  await ScrapDB.dbRun(
...
async function _rpcResetAllData() {
  await ScrapDB.dbRun(`DELETE FROM transactions`);
  const rows = await ScrapDB.dbQuery(`SELECT COUNT(*) as c FROM items`);
