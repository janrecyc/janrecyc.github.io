...
function renderFactorySelect() {
  ...
  if (!sel) return;
  const local = getLocalFactories();
  const source = local || (FACTORIES.length ? FACTORIES : [
...
//  FACTORY EDIT MODAL
// ══════════════════════════════════════════
let LOCAL_FACTORIES = null; 

function getLocalFactories() {
  if (LOCAL_FACTORIES !== null) return LOCAL_FACTORIES;
  try {
    const saved = localStorage.getItem('scrappos_factories');
    if (saved) return JSON.parse(saved);
  } catch(_) {}
  return null;
}

function saveLocalFactories(list) {
  LOCAL_FACTORIES = list;
  try { localStorage.setItem('scrappos_factories', JSON.stringify(list)); } catch(_) {}
}

function openFactoryEdit() {
  const sel = document.getElementById('factory-select');
  const factories = [...sel.options].map(o => {
    const raw  = o.text.replace('🏭 ', '');
    const dash = raw.indexOf(' — ');
    return {
      id:       o.value,
      name:     dash >= 0 ? raw.slice(0, dash) : raw,
      location: dash >= 0 ? raw.slice(dash + 3) : '',
    };
  });
...
function renderFactoryEditList(factories) {
  ...
  list.innerHTML = factories.map((f, i) => `
    <div class="fedit-item" id="fedit-row-${i}">
...
function saveFactoryEdit() {
  ...
  rows.forEach(row => {
    ...
    result.push({
      id:       isNew ? 'local_' + row.dataset.newid : row.querySelector('input')?.closest('.fedit-item')?.id?.replace('fedit-row-','') || idx,
      name,
      location: loc,
    });
  });

  if (!result.length) { toast('⚠️ ต้องมีโรงงานอย่างน้อย 1 แห่ง', 'error'); return; }

  const sel = document.getElementById('factory-select');
  const origOpts = [...sel.options];
  const finalList = result.map((f, fi) => {
    if (!String(f.id).startsWith('local_') && origOpts[fi]) {
      return { ...f, id: origOpts[fi].value };
    }
    return { ...f };
  });

  saveLocalFactories(finalList);

  const prevVal = sel.value;
  sel.innerHTML = finalList.map(f =>
    `<option value="${f.id}">🏭 ${f.name}${f.location ? ' — ' + f.location : ''}</option>`
  ).join('');
